...
# Import the new 2x2 tiling transform
from smalldiffusion.utils.data_transforms import tile_image_2x2, img_tile_2x2_transform
...
def main(args):
    ...
    if not args.disable_siren_cond:
        ...
        siren_config = dict(
            ndim=2,
            num_outs=1,
            # grid_size=args.image_size, # Use image_size arg - SIREN uses 28x28 grid
            grid_size=28, # SIREN model itself operates on 28x28 grid
    ...
    if accelerator.is_main_process and args.use_wandb:
        ...
        run_name = f"unet_tiled-{cond_suffix}-{siren_chkpt_name}" # Add tiled suffix
    ...
    checkpoint_path = os.path.join(save_dir, args.checkpoint_name.replace(".pth", "_tiled.pth")) # Add suffix
    ...
    # Use the new 2x2 tiling transform
    ...
    dataset = MappedDataset(MNIST(dataset_path, train=True, download=True,
                                  ...
                                  transform=img_tile_2x2_transform), # Use new 2x2 tiling
    ...
    for epoch in range(args.epochs):
        ...
        for batch in pbar:
            ...
            with accelerator.accumulate(model):
                ...
                noise = torch.randn_like(images) # images are now tiled [B, 1, 56, 56]
                # Create noisy image
                # x_noisy = images + noise * sigmas_broadcast
                x0_tiled = images # Rename for clarity
                x_noisy = x0_tiled + noise * sigmas_broadcast

                # --- Generate SIREN Condition (Conditional) ---
                siren_cond_tiled = None
                if not args.disable_siren_cond:
                    with torch.no_grad():
                        # Generate 28x28 SIREN output
                        siren_output_single = siren_model(labels)
                        # Tile the SIREN output to match image size (e.g., 56x56)
                        siren_cond_tiled = tile_image_2x2(siren_output_single)
                # --- End SIREN Condition ---
                ...
                if args.disable_siren_cond:
                    ...
                    # Ensure siren_cond_tiled is 4D [B, 1, H, W] before cat
                    if siren_cond_tiled is None or siren_cond_tiled.dim() != 4:
                        raise ValueError(f"Tiled SIREN condition has unexpected shape or is None: {siren_cond_tiled.shape if siren_cond_tiled is not None else 'None'}")
                    if x_noisy.shape != siren_cond_tiled.shape:
                         raise ValueError(f"Shape mismatch: x_noisy {x_noisy.shape}, siren_cond_tiled {siren_cond_tiled.shape}")
                    if x_noisy.dim() != 4:
                        raise ValueError(f"Noisy image has unexpected shape: {x_noisy.shape}")
                    unet_input = torch.cat([x_noisy, siren_cond_tiled], dim=1)
                # --- End UNet Input ---
...
if __name__ == '__main__':
    ...
    # --- UNet Model Params ---
    parser.add_argument('--image_size', type=int, default=56, help='Target tiled image size (e.g., 56 for 2x2 MNIST)')
    ...
    parser.add_argument('--checkpoint_name', type=str, default='mnist_unet_siren_cond_tiled.pth', help='Filename for the saved tiled UNet checkpoint')
